...
def add_date_analysis_columns(
    dataframe: pd.DataFrame,
) -> pd.DataFrame:
    """
    「実際日」から以下のCSV専用カラムを作る。
    ...
    day_numbers = actual_dates.dt.day

    result["日付"] = day_numbers.astype(
        "Int64"
    )
...
def main() -> None:
    ...
    export_dataframe = add_date_analysis_columns(
        export_dataframe
    )
